...
<details>
><summary><h2><a href='./theory_classify.md'>📘Теоритическая база</a></h2></summary>
Кликай по ~ТЕОРИТИЧЕСКАЯ БАЗА~
</details>
...
> Оба датасета доступны для загрузки напрямую через веб-интерфейс по ссылкам выше.
...
- **Python 3.12**
- **NVIDIA GPU** с поддержкой CUDA (рекомендуется для работы в реальном времени)
- Установленные NVIDIA-драйверы, совместимые с CUDA-версией PyTorch
...
2. Установите PyTorch с поддержкой CUDA. Версия команды зависит от вашей версии CUDA — точную команду можно получить на [pytorch.org/get-started](https://pytorch.org/get-started/locally/). Пример для CUDA 12.1:

```bash
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu121
```

3. Установите оставшиеся зависимости:

```bash
pip install ultralytics opencv-python
```

> Трекер `bytetrack.yaml` входит в состав пакета `ultralytics` и не требует отдельной установки.

### Структура проекта

Перед запуском убедитесь, что файлы весов и видео находятся в одной директории со скриптом `Patrol_AI`:
...
По умолчанию скрипт обрабатывает файл `1cars.mp4`. Чтобы запустить на другом видео, измените путь в строке:
...
Если CUDA-совместимая видеокарта недоступна, замените `.to('cuda')` на `.to('cpu')` в строках инициализации моделей:

```python
model = YOLO('Detection.pt').to('cpu')
classifier = YOLO('Classification.pt').to('cpu')
```

> ⚠️ На CPU обработка видео в реальном времени может быть значительно медленнее.
---
